%% This uses Life Cycle Model 9 to demonstrate how to solve for an unexpected (probability zero) change.
% I add a tax rate, tau, to Life-Cycle Model 9

% Idea: the tax rate is tau=0 and you expect this to last forever (for your whole life)
% But then one day (here at age 50, which is model period 31) you wake up
% and discover that the tax rate is tau=0.2, and that this will last
% forever (for your whole life).

% How to solve: life-cycle problems are always solved by backward
% iteration. 
% So first, tau=0.2 and we solve for the ages 50+ (we will solve whole life-cycle model, but just keep ages 50+).
% Second, we can now solve for ages 1-49, with tau=0 and a belief that this will continue forever.
% Third, splice the two (V & Policy) together (ages 50+ from first, with ages 1-49 from second).
% Fourth, we can just create the agent dist directly from this [note: expectations are 'encoded' in Policy, so we don't need to worry about them when creating AgentDist]
% Fifth, You need to be careful when computing any kind of moments from this.
...
% 'LifeCycleModel8_Tax_ReturnFn' just adds earnings tax tau
ReturnFn=@(h,aprime,a,z,w,sigma,psi,eta,agej,Jr,pension,r,kappa_j,tau,warmglow1,warmglow2,warmglow3,beta,sj) LifeCycleModel8_Tax_ReturnFn(h,aprime,a,z,w,sigma,psi,eta,agej,Jr,pension,r,kappa_j,tau,warmglow1,warmglow2,warmglow3,beta,sj);

%% First, solve for tau=0.2, and we will keep just the ages 50+ (so periods 31+)
Params.tau=0.2;
...
%% Second, solve for tau=0, and we will keep just the ages 20-49 (so periods 1-30)
Params.tau=0;
...
%% Third, splice these two together
V=zeros(n_a,n_z,N_j);
Policy=zeros(size(Policy1));
for jj=1:30
    V(:,:,jj)=V2(:,:,jj);
    Policy(:,:,:,jj)=Policy2(:,:,:,jj);
end
for jj=31:N_j
    V(:,:,jj)=V1(:,:,jj);
    Policy(:,:,:,jj)=Policy1(:,:,:,jj);
end

%% Fourth, we can just create the agent dist directly from this
...
%% Fifth, while creating model stats, be careful
Params.tau=[zeros(1,30),0.2*ones(1,N_j-30)]; % Use the tau values that prevailed in each period

%% FnsToEvaluate are how we say what we want to graph the life-cycles of
% Like with return function, we have to include (h,aprime,a,z) as first
% inputs, then just any relevant parameters.
FnsToEvaluate.fractiontimeworked=@(h,aprime,a,z) h; % h is fraction of time worked
FnsToEvaluate.aftertaxearnings=@(h,aprime,a,z,w,kappa_j,tau) (1-tau)*w*kappa_j*z*h; % w*kappa_j*z*h is the labor earnings (note: h will be zero when z is zero, so could just use w*kappa_j*h)
FnsToEvaluate.taxespaid=@(h,aprime,a,z,w,kappa_j,tau) tau*w*kappa_j*z*h; % w*kappa_j*z*h is the labor earnings (note: h will be zero when z is zero, so could just use w*kappa_j*h)
...
subplot(4,1,2); plot(1:1:Params.J,AgeConditionalStats.aftertaxearnings.Mean)
